...
def plot_results_overlay(start=0, stop=0):  # from utils.general import *; plot_results_overlay()
    ...
    for f in sorted(glob.glob('log*.txt') + glob.glob('../../Downloads/log*.txt')):
...
def plot_results(start=0, stop=0, bucket='', id=(), labels=(), save_dir=''):
    ...
    if bucket:
        ...
        files = ['log%g.txt' % x for x in id]
        c = ('gsutil cp ' + '%s ' * len(files) + '.') % tuple('gs://%s/log%g.txt' % (bucket, x) for x in id)
        os.system(c)
    else:
        files = glob.glob(str(Path(save_dir) / 'log*.txt')) + glob.glob('../../Downloads/log*.txt')
